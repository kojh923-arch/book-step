  ...
  if (e.target.id === 'auth-form') {
    ...
    try {
      if (state.authMode === 'signup') {
        const { error } = await sb.auth.signUp({ email, password, options: { data: { nickname } } }); if (error) throw error;
        state.error = '가입 완료! 이제 같은 닉네임과 비밀번호로 로그인해 주세요.'; state.authMode = 'login'; render(); return;
      }
