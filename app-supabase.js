...
const state = { view: 'home', authMode: 'login', pendingBook: null, mission: null, rating: 0, selectedRecord: null, error: '', levelUp: null, bookQuery: '', bookAuthor: '', bookResults: [], bookSearchError: '', bookSearching: false };
...
async function searchNaverBooks(query) {
  ...
  state.bookResults = [];
  render();
  const { data: result, error } = await sb.functions.invoke('naver-book-search', { body: { query } });
...
function auth() { return `<div class="auth-wrap"><div class="auth-card"><div class="brand"><span class="brand-mark">📚</span><div>독서한걸음<small>읽고 기록하고 성장해요</small></div></div><p class="eyebrow">MY READING SPACE</p><h1>${state.authMode === 'signup' ? '나만의 독서 기록을 시작해요' : '독서한걸음 불러오기'}</h1><p class="subtitle">닉네임과 비밀번호만으로, 어디서든 같은 기록을 이어갈 수 있어요.</p><div class="auth-toggle"><button class="${state.authMode === 'login' ? 'active' : ''}" data-action="login">로그인</button><button class="${state.authMode === 'signup' ? 'active' : ''}" data-action="signup">회원가입</button></div><form id="auth-form"><div class="field"><label for="nickname">닉네임</label><input id="nickname" required minlength="2" maxlength="16" placeholder="예: 책을 좋아하는 지현" /></div><div class="field"><label for="password">비밀번호</label><input id="password" type="password" required minlength="6" placeholder="6자 이상 입력" /></div>${state.authMode === 'signup' ? '<div class="field"><label for="class-code">학급코드</label><input id="class-code" required maxlength="30" placeholder="선생님에게 받은 학급코드" /></div>' : ''}<div id="auth-error" class="error" role="alert">${esc(state.error)}</div><button class="primary-button" style="width:100%" type="submit">${state.authMode === 'signup' ? '회원가입하고 시작하기' : '독서한걸음 불러오기'}</button></form><p class="notice">닉네임과 비밀번호를 잊지 않도록 꼭 기억해 주세요.</p></div></div>`; }
function recordForm() {
...
document.addEventListener('click', async e => {
  ...
  if (view) { state.view = view; state.pendingBook = null; state.mission = null; state.selectedRecord = null; if (view === 'record') { state.rating = 0; state.bookQuery = ''; state.bookAuthor = ''; state.bookResults = []; state.bookSearchError = ''; } render(); return; }
  ...
  if (action === 'select-book') { const index = Number(e.target.closest('[data-book-index]')?.dataset.bookIndex); const book = state.bookResults[index]; if (book) { state.bookQuery = book.title; state.bookAuthor = book.author; state.bookResults = []; state.bookSearchError = ''; render(); } return; }
